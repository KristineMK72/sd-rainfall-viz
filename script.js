...
async function fetchDaily(lat, lon) {
  const url = `https://archive-api.open-meteo.com/v1/archive?latitude=${lat}&longitude=${lon}&start_date=1940-01-01&end_date=2025-12-05&daily=precipitation_sum&precipitation_unit=inch&timezone=America/Chicago`;  // Fixed: end_date to today
  try {
...
async function buildChoropleth() {
  try {
    ...
    console.error("Critical Error: Failed to build Choropleth map. GeoJSON loading failed:", err);
  }
}
