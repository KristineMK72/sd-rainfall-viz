// ======================================================
// South Dakota Rainfall Explorer – script.js (REWRITTEN)
// ======================================================

// Import stats functions (Assumes these are correctly exported from stats.js)
...
// South Dakota FIPS code for filtering GeoJSON
const SD_FIPS_CODE = "46"; 

// GeoJSON URL for US County Boundaries (50m resolution)
// FIX: Using jsDelivr CDN to avoid CORS issues from raw.githubusercontent.com
const GEOJSON_URL = "https://cdn.jsdelivr.net/gh/datasets/geo-boundaries-us-counties/geojson/counties-50m.geojson";
...
/** Aggregates daily data into yearly sums. */
...
function onEachCounty(feature, layer) {
    ...
    layer.on({
        ...
        click: async () => {
            ...
            if (!data || !COUNTY_CENTROIDS[name]) return;
            map.fitBounds(layer.getBounds(), { padding: [50, 50] });
            updateChartFromCoords(COUNTY_CENTROIDS[name].lat, COUNTY_CENTROIDS[name].lon, `${name} County`);
        }
    });
...
async function buildChoropleth() {
    try {
        ...
        // Preload data for all known counties (in parallel)
        await Promise.all(Object.keys(COUNTY_CENTROIDS).map(loadCountyData));

        geoJsonLayer = L.geoJson(geojson, {
            filter: f => f.properties.STATE === SD_FIPS_CODE, // Filter by South Dakota FIPS
            style: styleCounty,
        ...
        console.error("Critical Error: Failed to build Choropleth map. GeoJSON loading failed:", error);
        // Display a fallback message on the map container
        document.getElementById('map').innerHTML = '<div style="padding: 20px; text-align: center; color: red;">Error: Failed to load map data. Check browser console for network errors.</div>';
    }
}
...
function updateLegend() {
    // Logic for updating the map legend remains the same
...
async function updateChartFromCoords(lat, lon, title) {
    ...
    updateStatsPanel(stats); // This updates the stats panel
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // 2. Map setup (This runs first because of the CSS fix)
    map = L.map("map").setView([44.37, -100.35], 7);
    L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
        attribution: "&copy; OpenStreetMap contributors"
    }).addTo(map);

    // 3. Build Choropleth Map and load data (The function with the GeoJSON fix)
